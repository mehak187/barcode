...
class GymController extends Controller
{
    public function newMember() {
        ...
        // print_r($members); print_r($resultnew);die();
        $data['results'] = $resultnew;
        
        $data['gym_id'] = $mid;
        $data['saveSendMail'] = mailInstruction::first();
        $data['saveSendPhone'] = phoneInstruction::first();
    ...
    //     'date' => $req->date,
    //    ]);
    //    return redirect('/member')->with('requestSuccess','Request sent successfully');
    // }
    public function saveRequestBarcode(Request $req) {
        RequestBarcode::create([
            'barcodes'=> $req->barcodes,
    ...
    public function ann(Request $req) {
        $annoucement = annoucement::where('id', 1)->first(); // Replace 'id' and '1' with the appropriate values
        if ($annoucement) {
            $annoucement->update([
    ...
    public function saveTiming(Request $req) {
     ...
     public function saveSendMail(Request $req) {
        $saveSendMail = mailInstruction::where('id', 1)->first(); // Replace 'id' and '1' with the appropriate values
        if ($saveSendMail) {
            $saveSendMail->update([
                'mailid'=> $req->mailid,
                'msg'=> $req->msg,
